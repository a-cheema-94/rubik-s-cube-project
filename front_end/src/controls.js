...
function getClosestFace(screenVector) {
  ...
  for (const face of PHYSICAL_FACES) {
    // Dot product calculation for how aligned current face is with the screen vector
    const alignment = face.vector.dot(screenVector);
...
function flipSlice(sliceChar) {
  return sliceChar === sliceChar.toLowerCase()
    ? sliceChar.toUpperCase()
    : sliceChar.toLowerCase();
}
