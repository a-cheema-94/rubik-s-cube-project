...
// read csv file
const csv = fs.readFileSync("./ml_section/data/pll_training_data.csv", "utf-8")
...
async function trainAndSaveModel() {
  ...
  await pll_model.fit(X, y, {
    epochs: 150,
    batchSize: 16,
  });

  // evaluate model
  const evalRes = pll_model.evaluate(X, y);
  ...
  const filepath = "file://./public/models/pll_model"

  await pll_model.save(filepath)
  ...
  // Run prediction
  const probabilities = pll_model.predict(sample_input);
